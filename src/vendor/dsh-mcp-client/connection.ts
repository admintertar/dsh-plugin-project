/*! Derived from DeepSeek MCP client, MIT. See THIRD_PARTY_NOTICES.md. */
/**
...
export function startConnection(ctx: Context, config: Config, policy: ResolvedReconnectPolicy,
  onState: (state: ConnectionState) => void = () => {}): ConnectionHandle {
  ...
  async function connectGeneration(startup: boolean): Promise<void> {
    ...
    const closed = {promise: new Promise<void>(resolve => { resolveClosed = resolve }), resolve: () => resolveClosed()}
