...
| `yarn run setup -- --desktop /path/to/official-source` | Export the pinned official runtime and install matching development packages |
...
`yarn run setup -- --shell /path/to/dsh-project-desktop` reads a prepared Shell's verified immutable sources. `yarn run test:compatibility -- /path/to/dsh-project-desktop` validates current plugin source and runs the Shell's checks. The Shell still uses its own pinned plugin snapshot: update its lock explicitly when adopting plugin changes. `yarn run desktop -- --shell /path/to/dsh-project-desktop` starts that Shell, not a mutable copy of the current plugin.
